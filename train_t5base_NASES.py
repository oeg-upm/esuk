...
class NewsSummaryDataset(Dataset):
  ...
  def __init__(self, data: pd.DataFrame, tokenizer: T5Tokenizer, text_max_token_len: int =768, summary_max_token_len: int=128):
...
class NewsSummaryDataModule(pl.LightningDataModule):
  ...
  def __init__(self, train_df: pd.DataFrame, test_df: pd.DataFrame, val_df: pd.DataFrame, tokenizer: T5Tokenizer, batch_size: int=4, text_max_token_len: int=768, summary_max_token_len: int=128):
     super().__init__()
...
def main():
  df = pd.read_json('./data/DACSA/spanish/train.json', lines=True)
  df=df[["article","summary"]]
  df.dropna()
  df1 = pd.read_json('./data/DACSA/spanish/test-i.json', lines=True)
  df1=df1[["article","summary"]]
  df1.dropna()
  df2 = pd.read_json('./data/DACSA/spanish/validation.json', lines=True)
  df2=df2[["article","summary"]]
  df2.dropna()
  df=df
  df1=df1
  df2=df2
  ...
  checkpoint_callback= ModelCheckpoint(dirpath= "checkpoints", filename="DASCA", save_top_k=2, verbose=True, monitor="val_loss", mode="min")
